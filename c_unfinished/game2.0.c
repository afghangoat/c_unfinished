...
#include <string.h>
/*Plot:
You are a zergling sent out by the overmind to 4pool a poor Command Center first terran.
Your mission is to navigate to the enemy base as fast as possible
TODO:
- seedsystem
*/
#ifdef _WIN32
...
void generatemap(){
    ...
    for(int k=0;k<4;k++){
        ...
        for(int i=0;i<rand()%10+20;i++){
            game_map[ty][tx]='.'; //cliff, unwalkable
            switch(rand()%4){
                case 0:
                    if(tx<19){
                        tx++;
	...
	game_map[hatchery_y][hatchery_x]='H';
}
void display_map(){
    for(int y=0;y<30;y++){
        for(int x=0;x<30;x++){
